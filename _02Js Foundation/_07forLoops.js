...
let visitedCities = [];
for (let c = 0; c < cityArr.length; c++) {
	if (cityArr[c] === "Paris") continue;
	visitedCities.push(cityArr[c]);
}
...
5. Use a `for-in` loop to loop through an object
	 ...
	 and store all previous cities' populations in a new
	 object named `cityPopulations`.
...
let teasList = ["earl grey", "green tea", "chai", "oolong tea"];
let availableTeas = [];

teasList.forEach(function (tea) {
    if (tea === 'chai') return;
...
let capitals = ["Berlin", "Tokyo", "Sydney", "Paris"];
let traveledCities = [];
capitals.forEach((city) => {
	if (city === 'Sydney') return; // skip
...
for (let p = 0; p < nums3.length; p++) {
	if (nums3[p] === 7) continue;
	doubledNumbers.push(nums3[p] * 2);
}
